<template>
	<div class="card bg-base-100 shadow">
		<div class="card-body">
			...
			<!-- View mode -->
			<p v-if="!editMode && note.notes" class="mt-2 whitespace-pre-wrap">{{ note.notes }}</p>
			<p v-else-if="!editMode && !note.notes" class="mt-2 text-sm text-base-content/70">No notes</p>

			<!-- Edit mode -->
			<form v-else class="grid gap-3" @submit.prevent="save">
				<label class="form-control w-full">
					<span class="label"><span class="label-text">Type</span></span>
					<select v-model="form.noteType" class="select select-bordered w-full">
		...
		<dialog ref="deleteDialogRef" class="modal">
			<div class="modal-box">
				<h3 class="font-bold text-lg">Delete flight note</h3>
				<p class="py-2">Are you sure you want to delete this note?</p>
...
import { ref } from 'vue'
import type { FlightNoteData, FlightNoteType, UpdateFlightNoteInput } from '@/api/rest/flight_notes.api'
import { updateFlightNote, deleteFlightNote } from '@/api/rest/flight_notes.api'

const props = defineProps<{ note: FlightNoteData }>()
const emit = defineEmits<{ (e: 'updated', value: FlightNoteData): void; (e: 'deleted', id: string): void }>()
...
const form = ref<UpdateFlightNoteInput>({})

function formatDate(iso: string): string { try { return new Date(iso).toLocaleString() } catch { return iso } }

function enableEdit() {
	editMode.value = true
	form.value = { noteType: props.note.noteType, notes: props.note.notes ?? undefined }
}
...
async function save() {
	try {
		saving.value = true
		error.value = ''
		const updated = await updateFlightNote(props.note.id, form.value)
		editMode.value = false
...
async function confirmDelete() {
	try {
		deleting.value = true
		error.value = ''
		await deleteFlightNote(props.note.id)
