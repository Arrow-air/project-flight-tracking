...
read_env_value() {
  ...
  if [[ -n "$line" ]]; then
    ...
    # If we got a value and it doesn't look encrypted (doesn't start with dotenvx:), return it
    if [[ -n "$value" ]] && [[ ! "$value" =~ ^dotenvx: ]]; then
      echo "$value"
      return 0
    fi
  fi
  ...
  if command -v dotenvx >/dev/null 2>&1 || command -v npx >/dev/null 2>&1; then
    ...
    if command -v dotenvx >/dev/null 2>&1; then
      ...
      temp_output=$(dotenvx run -f "$file" -- printenv "$key" 2>/dev/null || true)
    else
      # Fall back to npx dotenvx
      temp_output=$(npx --yes @dotenvx/dotenvx@latest run -f "$file" -- printenv "$key" 2>/dev/null || true)
    fi
